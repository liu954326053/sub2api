...
// The fork's generic API-key branch also matches OpenCode key rotations.
// Assert that OpenCode usage cleanup occurs before the generic branch.
func TestUpdateCredentialsOpenCodeBranchPrecedesOllamaBranch(t *testing.T) {
	client, mock := newOllamaCloudUsageRepositoryTestClient(t)
	mock.ExpectBegin()
	mock.ExpectExec(`(?s)UPDATE accounts.*platform = 'opencode_go'.*- 'opencode_go_usage_snapshot'.*WHEN type = 'apikey'\s+AND credentials IS DISTINCT FROM \$1::jsonb.*OR credentials ->> 'base_url' IS DISTINCT FROM \$1::jsonb ->> 'base_url'`).
		WithArgs(`{"api_key":"new-key","base_url":"https://opencode.ai/zen/go/v1"}`, int64(17)).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec(regexp.QuoteMeta("INSERT INTO scheduler_outbox")).
		WithArgs(service.SchedulerOutboxEventAccountChanged, int64(17), nil, nil, sqlmock.AnyArg()).
