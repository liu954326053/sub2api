...
func lockAndMergeAccountProbeExtra(
	...
	// 快照由上游倍率同步维护，与探测开关解耦：关闭自动探测只影响 probe runner
	// 资格与倍率回写开关，不再清除已同步的快照（否则快照会在每次编辑后被反复误清）。
...
func (r *accountRepository) UpdateCredentials(ctx context.Context, id int64, credentials map[string]any) error {
	...
	result, err := client.ExecContext(ctx, `
		...
		SET
			credentials = $1::jsonb,
			extra = CASE
				-- OpenCode identity cleanup must precede the generic API-key branch:
				-- a key rotation matches both and must also clear OpenCode usage state.
				WHEN type = 'apikey'
				...
				-- Fork: preserve synced snapshots unless the upstream identity changes.
				-- OpenCode identity cleanup above must run before this API-key branch.
				WHEN type = 'apikey'
					AND credentials IS DISTINCT FROM $1::jsonb
					AND (
						credentials -> 'api_key' IS DISTINCT FROM $1::jsonb -> 'api_key'
