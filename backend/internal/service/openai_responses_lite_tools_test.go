...
func TestNormalizeOpenAIResponsesLiteTools_ForcesSerialToolCalls(t *testing.T) {
	tests := []struct {
		name string
		body map[string]any
	}{
		...
		{
			name: "namespace additional tools",
			body: map[string]any{
				"input": []any{map[string]any{
					"type":  "additional_tools",
...
func TestOpenAIGatewayServiceForward_NormalizesResponsesLiteToolsForAPIKey(t *testing.T) {
	...
	account := &Account{
		...
		Concurrency: 1, Status: StatusActive, Schedulable: true, RateMultiplier: f64p(1),
		Credentials: map[string]any{"api_key": "api-token"},
