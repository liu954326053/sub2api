...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	// Resolve the billing multiplier against the credential (billing) account so
	// account_upstream billing mode uses the synced upstream rate.
	multiplier := 1.0
	if s.cfg != nil {
		multiplier = s.cfg.Default.RateMultiplier
	}
	if apiKey.GroupID != nil && apiKey.Group != nil {
		multiplier = s.ResolveUserGroupRateMultiplier(ctx, user.ID, *apiKey.GroupID, resolveBillingBaseMultiplier(apiKey.Group, billingAccount))
	}
